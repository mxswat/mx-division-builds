        ...
        for (let i = 0; i < enemyHealthByPlayerScaling.length; i++) {
            ...
            for (const difficulty in playerScalingData) {
                ...
                for (let j = 0; j < enemyTypes.length; j++) {
                    ...
                    const results = this.recursiveTimeToKillBulletToKill(weaponStats, enemyHP);
                    currentRow.push(`${results.timePassed.toFixed(1)}s / ${results.shotsFired.toFixed(0)} `);
                }
                rowData.push(currentRow);
            }
            weaponTTKData.push({ rowData, headers });
        }
        this._subjects[slot].next({ weaponName: weaponStats.weaponName, tables: weaponTTKData })
    }

    // Reduce enemyHP untill it's 0
    recursiveTimeToKillBulletToKill(weaponStats, enemyHP, shotsFired = 0, timePassed = 0, reloads = 0) {
        if (enemyHP <= weaponStats.dmgToOutOfCoverArmoredPerMag) {
            ...
            const remaningHP = enemyHP - weaponStats.dmgToOutOfCoverArmoredPerMag;
            shotsFired += weaponStats.totalMagSize;
            timePassed += weaponStats.timeToEmptyMagazine + weaponStats.reloadSpeed;
